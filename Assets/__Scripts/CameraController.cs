...
public class CameraController : MonoBehaviour
{
    ...
    private void Update() {
        if(Vector3.Distance(player1.transform.position, player2.transform.position) < 8 && !camRotate)
        {
            RotateCam();
            camRotate = true;
    ...
    IEnumerator ChangeRect(Camera obj, float time) 
    {
        ...
        if (temp > 0)
        {
            mainCamera.GetComponent<CinemachineVirtualCamera>().Priority = 20;
    ...
    void RotateCam()
    {
        StartCoroutine(RotateMe(player1.playerCam, Vector3.forward * -30,3f));
        StartCoroutine(RotateMe(player2.playerCam, Vector3.forward * -30,3f));
    }
